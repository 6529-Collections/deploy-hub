...
Assume current `main` is frozen as `M` and PR 1 is the tracked active staged
composition `A`. PR 2 and PR 3 have the same final target, so they share the
next snapshot `C23 = M + A + PR 2 + PR 3`. The workflow first separates an
infrastructure failure from a product failure.
...
flowchart TD
    ...
    B -- "No: product failure" --> E["Create non-force recovery commit R2<br/>content = M + A + PR 2"]
    ...
    G -- "Fail" --> I["Create non-force recovery commit R3<br/>content = M + A + PR 3"]
    ...
    L --> L1["PR 2: not staged<br/>product failure against retained composition"]
    K -- "Fail" --> M["Create non-force recovery commit RH<br/>content = M + retained tracked A"]
    M --> N["Canonical staging deploy + runtime proof + full E2E"]
    N --> O["PR 2 and PR 3 not staged<br/>staging restored to verified retained composition"]
```
